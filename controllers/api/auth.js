...
const { User } = require("../../models"); // This is a Sequelize model representing the User table in your PostgreSQL database. It’s imported from the models directory.
...
router.post("/signup", async (req, res) => {
  //  This route is for the POST method on the /signup path
  try {
    const { email, password, name } = req.body; // Shows that the applications expects to recieve the "password" and "email" from new user.
    const user = await User.create({ email, password, name }); // Creates the new user account using provided "password" and "email".(should be hashed before being stored in the database)
    req.session.userId = user.id; // stores new user data in the session. (they are considered logged in)
    res.status(201).send({ message: "User account created succefully!" }); // Following code sends a response depending on whether or not the user is succesfully logged in.
  } catch (error) {
    console.log(error);
...
router.post("/login", async (req, res) => {
  // Route for POST method on the '/Login' path.
  try {
    ...
    if (user && password === user.password) {
      // If a user is found, bcrypt.compare is used to compare the provided password with the hashed password stored in the database.
...
router.post("/logout", (req, res) => {
  // route for POST method on the '/logout' path.
  req.session.destroy((err) => {
    // This codes destroys/ends the session logging out the user.
    if (err) {
